...
class PrioritizedBuffer:
    ...
    def __init__(
            ...
            deepcopy: bool = False,
            start_unique_id: int = 0,
            is_demonstration: bool = False,
            demonstration_cfg: dict = {},
            eps: float = 0.01,
    ) -> int:
        r"""
        Overview:
        ...
        Arguments:
            ...
            - start_unique_id (:obj:`int`): Used to init ``self.latest_data_id``. Default set to 0. \
                For demonstration buffer, this should be 0; For agent buffer, this should be demo buffer's last id + 1.
            - is_demonstration (:obj:`bool`): Whether this buffer is a demonstration buffer, which is proposed \
                in R2D3, or an ordinary agent buffer, whose data are generated by self actor.
            - eps (:obj:`float`): A small positive number to avoid edge case
        """
        # TODO(nyz) remove elements according to priority
        self.is_demo = is_demonstration
        # current valid data count
        self._valid_count = 0
        # how many pieces of data have been pushed in this buffer, should be no less than ``_valid_count``
        self._push_count = 0
        ...
        self.latest_data_id = start_unique_id
        ...
        # for demonstration buffer, this attribute will be set to True after initialization
        self.forbid_append_extend = False
        if not self.is_demo:
            # oridinary prioritized buffer
            self._maxlen = maxlen
            ...
            # whether to deepcopy data when append/extend and sample data
            self._deepcopy = deepcopy
        else:
            # demonstration buffer
            self._enable_track_used_data = False
            demo_file_path = demonstration_cfg.load_path
            # first load demonstration data (should be a list) as _ori_data
            with open(demo_file_path, "rb") as f:
                _demo_ori_data = pickle.load(f)
            self._maxlen = len(_demo_ori_data)
            self._data = [None for _ in range(maxlen)]
            self._reuse_count = {idx: 0 for idx in range(maxlen)}  # {position_idx: reuse_count}
            self.max_reuse = max_reuse if max_reuse is not None else np.inf
            self.max_staleness = np.inf
            self.min_sample_ratio = 1.
            self.alpha = demonstration_cfg.get('alpha', 0.)
            assert (0 <= self.alpha <= 1)
            self._beta = demonstration_cfg.get('beta', 0.)
            self._anneal_step = 0  # todo
            # whether to deepcopy data when append/extend and sample data
            self._deepcopy = demonstration_cfg.get('deepcopy', False)
        ...
        # fill demonstration buffer with demonstration data using ``extend``
        if self.is_demo:
            self.extend(_demo_ori_data)
            self.forbid_append_extend = True
    ...
    def append(self, ori_data: Any) -> None:
        ...
        """
        if self.forbid_append_extend:
            raise Exception('Demonstration buffer is not allowed to append new data after initialization.')
        if self._deepcopy:
    ...
    def extend(self, ori_data: List[Any]) -> None:
        ...
        """
        if self.forbid_append_extend:
            raise Exception('Demonstration buffer is not allowed to extend new datas after initialization.')
        if self._deepcopy:
            data = copy.deepcopy(ori_data)
        else:
            data = ori_data
