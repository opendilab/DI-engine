...
from nervex.utils import EasyTimer, read_file
...
def entry(env, read_infer_ratio, use_cuda):
    ...
    out_str = '\n===== each_data: {:.4f}({}), infer: {:.4f}, read/infer: {:.4f}, batch_size: {}, chunk_size: {} ====='.format(
    ...
    #         print('\t\titer {:0>2d}, sum_time: {:.4f}, data_time: {:.4f}, infer_time: {:.4f}'.format(
    #             iter, sum_time, data_time, infer_time))
    #         iter += 1
    #         if iter == max_iter:
    #             break
    #     total_sum_time = total_data_time + total_infer_time
    #     out_str = '\ttotal_sum_time: {:.4f}, total_data_time: {:.4f}, total_infer_time: {:.4f}, data/sum: {:.4f}'.format(
    ...
    # out_str = '\t(Our DataLoader {} average) total_sum_time: {:.4f}, total_data_time: {:.4f}, total_infer_time: {:.4f}, data/sum: {:.4f}'.format(
    ...
    for real_num_workers in [0]:
        ...
        total_data_time = sum(total_data_time_list) / len(total_data_time_list)
        total_infer_time = sum(total_infer_time_list) / len(total_infer_time_list)
        out_str = '\t(PyTorch DataLoader {} average) total_sum_time: {:.4f}, total_data_time: {:.4f}, total_infer_time: {:.4f}, data/sum: {:.4f}'.format(
            exp_times, total_sum_time, total_data_time, total_infer_time, total_data_time / total_sum_time)
        out_str_list.append(out_str)
        print(out_str)
