...
class SumoDqnLoss(BaseLoss):
    ...
    def __init__(self, agent, discount_factor: Optional[float] = 0.99, q_function_criterion=nn.MSELoss()):
        self.agent = agent
    ...
    def compute_loss(self, data: dict):
        ...
        action_batch = torch.cat([torch.IntTensor([x['acts']]) for x in batchs])
