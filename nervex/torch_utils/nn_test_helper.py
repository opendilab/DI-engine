...
def is_differentiable_print_no_grad(loss: torch.Tensor, model: Union[torch.Tensor, List[torch.Tensor]]) -> None:
    """
    Overview:
        Judge whether the model/models are differentiable. First check whether module's grad is None,
        then do loss's back propagation, finally check whether module's grad are torch.Tensor, if not then print it.
    Arguments:
        - loss (:obj:`torch.Tensor`): loss tensor of the model
        - model (:obj:`Union[torch.Tensor, List[torch.Tensor]]`): model or models to be checked
    """
