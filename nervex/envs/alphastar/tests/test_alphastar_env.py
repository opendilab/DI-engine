...
import numpy as np
import torch
from nervex.data.fake_dataset import fake_stat_processed_professional_player, get_random_action
...
class TestAlphaStarEnv:
    def _get_random_action(self, agent_num, map_size):
        def action_fn():
            ...
            selected_units = action['selected_units'].tolist() if isinstance(action['selected_units'],
                                                                             torch.Tensor) else []
        ...
        return [action_fn() for _ in range(agent_num)]
