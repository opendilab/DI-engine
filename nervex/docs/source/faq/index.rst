...
:A3:
  - 如果不使用nervex中Armor, Adder等抽象模块，完全自定义实现策略，则神经网络只需要继承torch.nn.Module实现即可，没有任何其他要求。
  - 如果使用上述模块，则神经网络的输入输出一般情况下 **必须** 为dict，且dict中相关键值对的键必须满足一些限制，具体可以参考armor部分的相关文档。
