...
Replay Buffer
^^^^^^^^^^^^^^^
Replay buffer is a component to store data collected by collector, then provide data for learner to optimize policy. In nerveX, there are **three types of replay buffers**:
...
The key methods of a buffer are ``push`` and ``sample``. ``NaiveReplayBuffer`` is a simple FIFO queue implementation. It only provides basic functions of the two methods.
   ...
   2. ``sample``: Randomly sample a list with length `batch_size`.
...
``AdvancedReplayBuffer`` implements following features: (Also shown in figure)
   ...
   - **Monitor data attributes(`use_count` and `staleness`)**. Count how many times a piece of data is sampled. Record the model iteration gap between the time it is collected and the time it is sampled. If a piece of data is used too many times, or is too stale to optimize policy, it will be removed out of buffer.
...
``EpisodeReplayBuffer`` is designed for some special cases where they need a whole episode rather than seperated samples. For example: In chess, go or card games, players get reward only when the game is over; In some algorithms like Hindsight Experience Replay, must sample out a whole episode and operate on it. Therefore, in ``EpisodeReplayBuffer``, each element is no longer a train sample, but an episode.
