...
其中 :math:`\delta_tV \overset{def}{=} \rho_t(r_t+\gamma V(x_{t+1})-V(x_t))` ，
...
Critic的更新方式为最小化 :math:`V_\theta (x)` 相对于 :math:`v_s` 的均方误差，即为：:math:`(v_s-V_\theta (x))\nabla_\theta V_\theta (x)` 。

Actor的参数 :math:`\omega` 需要朝着off-policy policy gradient给出的梯度方向更新，
即 :math:`\mathbb{E}[\frac{\pi(a_i|x_i)}{\mu(a_i|x_i)} \nabla log \pi Q^\pi(x_s,a_s)]` 。
其中：
