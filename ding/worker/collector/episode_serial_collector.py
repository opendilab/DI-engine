...
class EpisodeSerialCollector(ISerialCollector):
    ...
    def reset(
        ...
        self._traj_buffer = {
            env_id: TrajBuffer(maxlen=self._traj_len) for env_id in range(self._env_num)
        }
        self._env_info = {
    ...
    def collect(
        ...
        ), "Please make sure n_episode >= env_num{}/{}".format(n_episode, self._env_num)
        ...
        while True:
            with self._timer:
                ...
                ready_env_id = ready_env_id.union(
                    set(list(new_available_env_id)[:remain_episode])
                )
