...
from ding.framework.middleware import OffPolicyLearner, envpool_evaluator, data_pusher, \
    ...
    termination_checker, wandb_online_logger, epoch_timer, EnvpoolStepCollectorV2, OffPolicyLearnerV2, OffPolicyLearnerV3, OffPolicyLearnerV4
...
def main(cfg):
    ...
    cfg.exp_name = 'Test-Pong-v5-DQN-envpool-new-' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    with task.start(async_mode=False, ctx=OnlineRLContext()):
        ...
        task.use(OffPolicyLearnerV4(cfg, policy, buffer_))
