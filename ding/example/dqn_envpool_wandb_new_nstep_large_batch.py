...
from dizoo.atari.config.serial import pong_dqn_envpool_large_batch_config


def main(cfg):
    logging.getLogger().setLevel(logging.INFO)
    cfg.exp_name = 'Pong-v5-DQN-envpool-large_batch-' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
if __name__ == "__main__":
    ...
    pong_dqn_envpool_large_batch_config.env.collector_env_num = arg.collector_env_num
    pong_dqn_envpool_large_batch_config.env.collector_batch_size = arg.collector_batch_size
    pong_dqn_envpool_large_batch_config.seed = arg.seed
    pong_dqn_envpool_large_batch_config.env.stop_value = 2000
    pong_dqn_envpool_large_batch_config.nstep = 3
    pong_dqn_envpool_large_batch_config.policy.nstep = 3
    pong_dqn_envpool_large_batch_config.seed = arg.seed

    main(pong_dqn_envpool_large_batch_config)
