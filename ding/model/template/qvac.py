...
@MODEL_REGISTRY.register('discrete_qvac')
class DiscreteQVAC(nn.Module):
    """
    Overview:
        The neural network and computation graph of algorithms related to discrete action Actor-Critic that have both Q-value and V-value critic, \
        such as Discrete action IQL. This model now supports only discrete action space. The DiscreteQVAC is composed of \
        four parts: ``actor_encoder``, ``critic_encoder``, ``actor_head`` and ``critic_head``. Encoders are used to \
    ...
    def __init__(
        ...
        """
        Overview:
            Initailize the DiscreteQVAC Model according to input arguments.
        Arguments:
            - obs_shape (:obj:`Union[int, SequenceType]`): Observation's shape, such as 128, (156, ).
            - action_shape (:obj:`Union[int, SequenceType, EasyDict]`): Action's shape, such as 4, (3, ).
            ...
            - share_encoder (:obj:`Optional[bool]`): Whether to share encoder between actor and critic.
        """
        super(DiscreteQVAC, self).__init__()
        obs_shape: int = squeeze(obs_shape)
    ...
    def forward(self, inputs: torch.Tensor, mode: str) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            - output (:obj:`Dict[str, torch.Tensor]`): The output dict of QVAC forward computation graph, whose \
                key-values vary in different forward modes.
        Examples (Actor):
            >>> model = DiscreteQVAC(64, 6)
            >>> obs = torch.randn(4, 64)
            >>> actor_outputs = model(obs,'compute_actor')
            >>> assert actor_outputs['logit'].shape == torch.Size([4, 6])

        Examples(Critic):
            >>> model = DiscreteQVAC(64, 6, twin_critic=False)
            >>> obs = torch.randn(4, 64)
    ...
    def compute_actor(self, inputs: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        Arguments:
            - inputs (:obj:`torch.Tensor`): The input observation tensor data.
        Returns:
            - outputs (:obj:`Dict[str, torch.Tensor]`): The output dict of QVAC forward computation graph for actor, \
    ...
    def compute_critic(self, inputs: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Overview:
            QVAC forward computation graph for critic part, input observation to predict Q-value for each possible \
            discrete action choices.
        Arguments:
            - inputs (:obj:`torch.Tensor`): The input observation tensor data.
        Returns:
            - outputs (:obj:`Dict[str, torch.Tensor]`): The output dict of QVAC forward computation graph for critic, \
        ...
        if self.twin_critic:
            x = [m(inputs)['logit'] for m in self.critic_q_head]
            y = [m(inputs)['logit'] for m in self.critic_v_head]
        else:
            x = self.critic_q_head(inputs)['logit']
            y = self.critic_v_head(inputs)['logit']
        return {'q_value': x, 'v_value': y}
