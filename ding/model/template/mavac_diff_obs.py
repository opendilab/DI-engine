...
class DictEncoder(nn.Module):
    def __init__(self, unique_lengths, out_size):
        # unique_lengths: list of value len in dict
        super(DictEncoder, self).__init__()
        self.length_list = unique_lengths
        self.encoders = nn.ModuleList([
            nn.Linear(in_size, out_size) for in_size in unique_lengths
        ])

    def forward(self, x, length):
        index = self.length_list.index(length)
        encoder = self.encoders[index]
        return encoder(x)

@MODEL_REGISTRY.register('mavacdo')
class MAVACDO(nn.Module):
    r"""
    Overview:
        The MAVAC model.
    Interfaces:
    ...
    def __init__(
        self,
        agent_obs_shape: Union[int, dict, SequenceType],
        global_obs_shape: Union[int, dict, SequenceType],
        action_shape: Union[int, SequenceType],
        num_good: int,
        num_adversaries: int,
        num_obstacles: int,
        actor_encoder_hidden_size = 256,    # The length after uniform encoding of obs in the dict
        ...
        """
        super(MAVACDO, self).__init__()
        if type(agent_obs_shape) is dict:
            agent_obs_lengths = agent_obs_shape.values()
        else:
            agent_obs_lengths = agent_obs_shape
        if type(global_obs_shape) is dict:
            global_obs_lengths = global_obs_shape.values()
        else:
            global_obs_lengths = global_obs_shape
        agent_obs_unique_lengths = list(set(agent_obs_lengths))     # Statistics of different lengths in agent_obs
        global_obs_unique_lengths = list(set(global_obs_lengths))
        ...
        self.actor_encoder = DictEncoder(agent_obs_unique_lengths, actor_encoder_hidden_size)
        self.critic_encoder = DictEncoder(global_obs_unique_lengths, critic_encoder_hidden_size)
        # Head Type, only consider discrete action
        self.critic_head = nn.Sequential(
            nn.Linear(critic_encoder_hidden_size, critic_head_hidden_size), activation,
            RegressionHead(
                critic_head_hidden_size, 1, critic_head_layer_num, activation=activation, norm_type=norm_type
            )
        )
        self.actor_head = nn.Sequential(
            nn.Linear(actor_encoder_hidden_size, actor_head_hidden_size), activation,
            DiscreteHead(
    ...
    def compute_actor(self, x: torch.Tensor) -> Dict:
        ...
        """
        # only consider discrete action
        action_mask = x['action_mask']
        x = x['agent_state']    # x['agent_state'] is a dict
        encoded_agents_obs = []
        for agent, agent_obs in x.items():
            encoded_agent_obs = self.actor_encoder(agent_obs, agent_obs.shape[1])
            encoded_agents_obs.append(encoded_agent_obs)
        x = torch.stack(encoded_agents_obs, dim=1)
        x = self.actor_head(x)
        logit = x['logit']
    ...
    def compute_critic(self, x: Dict) -> Dict:
        ...
        encoded_global_obs = []
        for agent, agent_obs in x.items():
            encoded_obs = self.critic_encoder(agent_obs, agent_obs.shape[1])
            encoded_global_obs.append(encoded_obs)
        x = torch.stack(encoded_global_obs, dim=1)
