...
class NGU(nn.Module):
    ...
    def forward(
       ...
       ArgumentsKeys:
        ...
        # prev_action, prev_reward_extrinsic,  prev_areward_i =
        # inputs['prev_action'], inputs['prev_reward_extrinsic'], inputs['prev_reward_i']
        ...
        if inference:  # collect,eval
            ...
            # if nstep:
            prev_reward_extrinsic = prev_reward_extrinsic[:, :, 0].unsqueeze(-1)  # 20,32,1
            env_num = self.collector_env_num
            ...
            x_a_r_beta = torch.cat([x, prev_action_onehot, prev_reward_extrinsic, beta_onehot], dim=-1)  # 20,32,80
            x = x_a_r_beta
            lstm_embedding = []
