...
from cv2 import arcLength, initUndistortRectifyMap
from numpy import block, printoptions
import pytest
import torch
from ding.torch_utils.network import resnet18
from ding.torch_utils.network.resnet import ResNet, BasicBlock, Bottleneck, AvgPool2dSame, avg_pool2d_same, ClassifierHead
...
output_stride = [8,32]
num_classes = [0, 1000]
args = [item for item in product(*[stem_type, replace_stem_pool, avg_down, block, layers, zero_init_last_bn, output_stride, num_classes])]


@pytest.mark.unittest
@pytest.mark.parametrize('stem_type, replace_stem_pool, avg_down, block, layers, zero_init_last_bn, output_stride, num_classes', args)
def test_resnet(stem_type, replace_stem_pool, avg_down, block, layers, zero_init_last_bn, output_stride, num_classes):
    model = ResNet(stem_type=stem_type, replace_stem_pool=replace_stem_pool, avg_down=avg_down, block=block, layers=layers, 
        output_stride=output_stride, num_classes=num_classes, drop_rate=0. if stem_type=='deep' else 0.05)
