...
class ScatterConnection(nn.Module):
    ...
    def __init__(self, scatter_type: str) -> None:
        ...
        Arguments:
            - scatter_type (:obj:`str`): The scatter type, which decides the behavior when two entities have the same location.
                It can be either 'add' or 'cover'. If 'add', the first one will be added to the second one. If 'cover',
                the first one will be covered by the second one.
        """
    ...
    def forward(self, x: torch.Tensor, spatial_size: Tuple[int, int], location: torch.Tensor) -> torch.Tensor:
        ...
        Arguments:
            - x (:obj:`torch.Tensor`): The input tensor of shape `(B, M, N)`, where `B` is the batch size, `M` is the number of entities,
                and `N` is the dimension of entity attributes.
            - spatial_size (:obj:`Tuple[int, int]`): The size `(H, W)` of the spatial feature map into which 'x' will be scattered,
