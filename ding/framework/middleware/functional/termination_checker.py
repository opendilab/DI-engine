...
def termination_checker(max_env_step: Optional[int] = None, max_train_iter: Optional[int] = None) -> Callable:
    ...
    def _check(ctx: Union["OnlineRLContext", "OfflineRLContext"]):
        ...
        if hasattr(ctx, "trained_env_step") and ctx.trained_env_step > max_env_step:
            task.finish = True
            logging.info('Exceeded maximum number of env_step({}), program is terminated'.format(ctx.trained_env_step))
