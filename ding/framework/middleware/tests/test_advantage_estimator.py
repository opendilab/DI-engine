...
from ding.framework.middleware.functional.advantage_estimator import gae_estimator, collate
...
def call_gae_estimator(batch_size: int = 32, trajectory_end_idx_size: int = 5, buffer: Optional[Buffer] = None):
    ...
    ctx.trajectories_copy = collate(copy.deepcopy(ctx.trajectories))
    ...
    if buffer is not None:
        ...
        for d in train_data:
            ...
            d.obs = d.obs[0]
        ctx.train_data = collate(train_data)
