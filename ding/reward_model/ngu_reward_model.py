...
class EpisodicRewardModel(BaseRewardModel):
    ...
    def _train(self) -> None:
        ...
        train_index = np.random.randint(low=0,high= self.train_obs.shape[0],size=64) # self.cfg.reward_model_batch_size) 
    ...
    def train(self) -> None:
        ...
        if isinstance(self.cfg.obs_shape ,int):
            self.train_obs = torch.stack(self.train_obs,dim=0).view(len(self.train_obs)*len(self.train_obs[0]),self.cfg.obs_shape) # -1) TODO image
        ...
        self.train_next_obs = torch.stack(self.train_next_obs,dim=0).view(len(self.train_next_obs)*len(self.train_next_obs[0]),-1) 
        self.train_action = torch.stack(self.train_action,dim=0).view(len(self.train_action)*len(self.train_action[0]),-1)
    ...
    def _compute_intrinsic_reward(self, episodic_memory: List,
        ...
        s = np.sqrt(np.max(np.sum(kernel),0)) + c  #TODO
    ...
    def estimate(self, data: list) -> None:
        ...
        # stack batch dim
        if isinstance(self.cfg.obs_shape ,int):
            obs = torch.stack(obs,dim=0).view(batch_size *timesteps,self.cfg.obs_shape).to(self.device) # -1) TODO image
        else:#:len(self.cfg.obs_shape) == 3
            obs = torch.stack(obs,dim=0).view(batch_size *timesteps,*self.cfg.obs_shape).to(self.device) # -1) TODO image
        ...
        with torch.no_grad():
            ...
            episodic_reward = (episodic_reward - episodic_reward.min()) / (episodic_reward.max() - episodic_reward.min() + 1e-8)  # TODO
