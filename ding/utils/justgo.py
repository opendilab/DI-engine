...
from multiprocessing import Process, Manager,Pipe, connection, get_context, Array, cpu_count
import ctypes
from utils import *
import pyarrow
import subprocess
    ...
    def test_plasma_pyarrow(self,cpu_count):
        with Tick('test'):
            ...
            with Tock('mp'):
                processes = []
                for i in range(cpu_count):
                    p = mp.Process(target=self.job_3,args=(tmp,i,))
                    p.start()
                    processes.append(p)
                for precess in processes:
                    precess.join()
    ...
    def test_plasma_pickle(self,cpu_count):
        with Tick('test'):
            ...
            with Tock('mp'):
                ...
                #print(tmp)
                for i in range(cpu_count):
                    p = mp.Process(target=self.job_4,args=(tmp,i,))
                    p.start()
                    processes.append(p)
                for precess in processes:
                    precess.join()
...
if __name__ == '__main__':
    ...
    #path = "/Users/yangzhenjie/code/DI-engine/EfficiencyOptimization/data/ckpt_best.pth.tar"
    #data = torch.load(path,map_location='cpu')
    #data = to_ndarray(data)
    #print(data)

    cpu_count = 6
    MT = Serialized_Zoo(data_2dot5G)
    #MT.pickle_cost()
    #MT.pyarrow_cost()
