...
class SerializedZoo:
    ...
    def dump(self):
        ...
        if self.type == 'mp_pickle':
            return MultiprocessingPickle(
                self.var, self.shape, self.cpu_count, mode='dump', byte_length=self.byte_length
            ).pack()

        if self.type == 'mp_pyarrow':
            return MultiprocessingPyarrow(
                self.var, self.shape, self.cpu_count, mode='dump', byte_length=self.byte_length
            ).pack()
    ...
    def load(self):
        ...
        if self.type == 'mp_pickle':
            return MultiprocessingPickle(
                self.var, self.shape, self.cpu_count, mode='load', byte_length=self.byte_length
            ).unpack()
