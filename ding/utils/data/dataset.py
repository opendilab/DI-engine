    ...
    def __init__(self, dataset_path: str, context_len: int, rtg_scale: float) -> None:
        ...
        if isinstance(self.trajectories[0], list):
            ...
            for key, o_key in zip(keys, original_keys):
                trajectories_tmp = [{key: np.stack([self.trjectories[eps_index][transition_index][o_key] for transition_index in range(len(self.trajectories[eps_index]))], axis=0)} for eps_index in range(len(self.trajectories))]
