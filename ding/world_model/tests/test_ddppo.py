...
class TestDDPPO:
    ...
    def get_world_model(self, state_size, action_size, reward_size):
        cfg = DDPPOWorldMode.default_config()
        cfg = deep_merge_dicts(
            cfg, dict(cuda=False, model=dict(state_size=state_size, action_size=action_size, reward_size=reward_size))
        )
    ...
    def test_get_jacobian(self, state_size, action_size, reward_size):
        ...
        assert jacobian.shape == (ensemble_size, B, state_size + reward_size, state_size + action_size)
        assert jacobian.requires_grad

    @pytest.mark.parametrize('state_size, action_size, reward_size', args)
    def test_step(self, state_size, action_size, reward_size):
        ...
        assert rewards.shape == (128, reward_size)
    ...
    @pytest.mark.parametrize('state_size, action_size, reward_size', args)
    def test_train_graident_model(self, state_size, action_size, reward_size):
        states = torch.rand(1280, state_size)
        actions = torch.rand(1280, action_size)
