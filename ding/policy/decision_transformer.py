...
class DTPolicy(DQNPolicy):
    ...
    def _init_learn(self) -> None:
        ...
        batch_size = self._cfg.batch_size            # training batch size
        ...
        num_updates_per_iter = self._cfg.num_updates_per_iter
    ...
    def _forward_learn(self, data_iter,traj_data_loader) -> Dict[str, Any]:
        r"""
            Overview:
    ...
    def evaluate_on_env(self, state_mean=None, state_std=None, render=False):
        ...
        with torch.no_grad():
            ...
            for _ in range(self.num_eval_ep):
                ...
                # zeros place holders
                # continuous action
                actions = torch.zeros((eval_batch_size, self.max_test_ep_len, self.act_dim),
                ...
                states = torch.zeros((eval_batch_size, self.max_test_ep_len, self.state_dim),
                                    dtype=torch.float32, device=self.device)
                rewards_to_go = torch.zeros((eval_batch_size, self.max_test_ep_len, 1),
                                    dtype=torch.float32, device=self.device)
                ...
                for t in range(self.max_test_ep_len):
                    ...
                    if t < self.context_len:
                        _, act_preds, _ = self._learn_model.forward(timesteps[:,:self.context_len],
                                                    states[:,:self.context_len],
                                                    actions[:,:self.context_len],
                                                    rewards_to_go[:,:self.context_len])
                        act = act_preds[0, t].detach()
                    else:
                        _, act_preds, _ = self._learn_model.forward(timesteps[:,t-self.context_len+1:t+1],
                                                    states[:,t-self.context_len+1:t+1],
                                                    actions[:,t-self.context_len+1:t+1],
                                                    rewards_to_go[:,t-self.context_len+1:t+1])
    ...
    def evaluate(self, state_mean=None, state_std=None, render=False): 
        ...
        if eval_d4rl_score >= max_d4rl_score:
            ...
            torch.save(self._learn_model.state_dict(), save_best_model_path)
            max_d4rl_score = eval_d4rl_score
        print("saving current model at: " + save_model_path)
        torch.save(self._learn_model.state_dict(), save_model_path)
