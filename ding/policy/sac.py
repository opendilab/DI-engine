...
from ding.rl_utils import v_1step_td_data, v_1step_td_error, get_train_sample, q_v_1step_td_error, q_v_1step_td_data, generalized_lambda_returns
...
class MBSACPolicy(SACPolicy):
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        target_q_values = evaluation(obss, actions, self._target_model)
        if self._twin_critic:
        ...
        # critic_loss
        q_values = evaluation(obss.detach(), actions.detach(), self._learn_model)
        if self._twin_critic:
