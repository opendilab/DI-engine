...
class ContinuousBCOPolicy(Policy):
    ...
    def _init_learn(self):
        ...
        def lr_scheduler_fn(epoch):
            if epoch <= self._cfg.learn.warmup_epoch:
                ...
                ratio = (epoch - self._cfg.learn.warmup_epoch) // self._cfg.learn.decay_epoch
                return max(1e-5, math.pow(self._cfg.learn.decay_rate, ratio))
            # return self._cfg.learn.learning_rate
    ...
    def _forward_learn(self, data):
        ...
        with self._timer:
            ...
            # action_pred = torch.tanh(dist.rsample())
            action_pred = self._learn_model.forward(data['obs'], mode='compute_actor')['action']
            loss = self._loss(action_pred, action.detach())
