...
class AlphaZeroPolicy(Policy):
    def _init_learn(self):
        # Optimizer
        self._grad_norm = self._cfg.learner.get('grad_norm', 1)
        ...
        self._update_per_collect = self._cfg.learner.get('update_per_collect', 2)
    ...
    def train_step(self, state_batch, mcts_probs, winner_batch, learning_rate):
        ...
        return total_loss.item(), entropy.item(), grad_norm
    ...
    def _forward_learn(self, inputs: dict) -> Dict[str, Any]:
        ...
        for _ in range(self._update_per_collect):
            total_loss, entropy, grad_norm = self.train_step(state_batch, mcts_probs, winner_batch,
