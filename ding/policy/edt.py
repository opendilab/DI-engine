...
class EDTPolicy(Policy):
    ...
    Overview:
        ...
        Paper link: https://arxiv.org/abs/2307.02484
    """
    config = dict(
    ...
    def _forward_learn(self, data: List[torch.Tensor]) -> Dict[str, Any]:
        ...
        if self._atari_env:
            state_preds, action_preds, return_preds, return_preds2, reward_preds = self._learn_model.forward(
    ...
    def _forward_eval(self, data: Dict[int, Any]) -> Dict[int, Any]:
        ...
        with torch.no_grad():
            ...
            if self._basic_discrete_env:
                actions = actions.squeeze(-1)
            _, act_preds, _, _, _= self._eval_model.forward(timesteps, states, actions, rewards_to_go)
