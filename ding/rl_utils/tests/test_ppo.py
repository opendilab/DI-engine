...
def test_ppo_error_continous(use_value_clip, dual_clip, weight):
    B, N = 4, 1
    mu_sigma_new = torch.randn(B, N).requires_grad_(True)
    mu_sigma_old = mu_sigma_new + torch.rand_like(mu_sigma_new) * 0.1
    ...
    assert all([l.shape == tuple() for l in loss])
    assert all([np.isscalar(i) for i in info])
    assert mu_sigma_new.grad is None
    assert value_new.grad is None
    total_loss = sum(loss)
    total_loss.backward()
    assert isinstance(mu_sigma_new.grad, torch.Tensor)
    assert isinstance(value_new.grad, torch.Tensor)
