...
In order to get the entire config, we have *compile*, a bottom-up process. First we get the default setting
...
On the other hand, *initialization*, the process to pass config, is from top to bottom. We will start from
policy and env, and then pass configs to each submodule.
...
Rules when merging user-specified config and predefined config:

-  User-specified config is of highest priority, which means that it will cover
   the predefined one when conflict occurs.

-  Some important keys, such as ``env.stop_value``, ``policy.on_policy``,
   and ``policy.unroll_len``  must be specified.

-  The merged entire config will be saved to ``formatted_total_config.py``  by default.
