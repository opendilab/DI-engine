...
def main(cfg: dict, create_cfg: dict, seed: int, env_setting: Optional[List[Any]] = None) -> None:
    ...
    cfg = compile_config(
        cfg, seed=seed, policy=BehaviourCloningObservationPolicy, auto=True, create_cfg=create_cfg, save_cfg=True
    )
    ...
    state_trainsition_model = nn.Sequential(
        nn.Linear(cfg.policy.model.obs_shape * 2, 60), nn.ReLU(), nn.Linear(60, 80), nn.ReLU(), nn.Linear(80, 100),
        nn.ReLU(), nn.Linear(100, 70), nn.ReLU(), nn.Linear(70, 40), nn.ReLU(), nn.Linear(40, 10), nn.ReLU(),
    ...
    if cfg.policy.collect.demonstration_model_path is None:
        policy = BehaviourCloningObservationPolicy(
            cfg.policy, model=policy_model, enable_field=['learn', 'collect', 'eval']
        )
    ...
    for epoch in range(cfg.policy.learn.train_epoch):
        ...
        expert_learn_dataset_obj = CustomDataset(
