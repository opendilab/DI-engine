...
def main(cfg, model, seed=0):
    with Task(async_mode=True) as task:
        ...
        task.use(
            task.sequence(td3.act(collector_env), td3.collect(collector_env, replay_buffer, task=task)),
            filter_labels=["standalone", "node.[1-9]*"]
        )
        task.use(td3.learn(replay_buffer, task=task), filter_labels=["standalone", "node.0"])
