...
from dizoo.atari.config.serial.space_invaders.spaceinvaders_sqil_config import main_config, create_config
from dizoo.atari.config.serial.space_invaders.spaceinvaders_sqil_config import space_invaders_sqil_config
...
def serial_pipeline_sqil(
    ...
    expert_policy.collect_mode.load_state_dict(torch.load('/home/SENSETIME/nieyunpeng/nerveX/dizoo/atari/config/serial/pong/This_is_demo_data_pong/ckpt/ckpt_best.pth.tar', map_location='cpu'))
    ...
    for _ in range(max_iterations):
        ...
        for i in range(len(new_data)):
            new_data[i]['reward'] = torch.tensor([0., 0., 0.])
            expert_data[i]['reward'] = torch.tensor([1., 1., 1.])
