...
class Policy(ActorCriticBase):
    ...
    def _actor_forward(self, inputs, temperature=1.0):
        ...
        # action type
        logits['action_type'], actions['action_type'], embeddings = self.head['action_type_head'](
        ...
        # action arg delay
        logits['delay'], actions['delay'], embeddings = self.head['delay_head'](embeddings)

        for idx in range(actions['action_type'].shape[0]):
            embedding = embeddings[idx:idx+1]
            # action arg queued
