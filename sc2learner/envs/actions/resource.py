...
class ResourceActions(object):
    ...
    def action_assign_workers_gather_gas(self):
        return Function(
            name="assign_workers_gather_gas",
    ...
    def action_assign_workers_gather_minerals(self):
        return Function(
            name="assign_workers_gather_minerals",
    ...
    def _all_idle_queens_inject_larva(self, dc):
        injectable_queens = [
            ...
            u for u in dc.units_of_type(UNIT_TYPE.ZERG_QUEEN.value) if u.float_attr.energy >= 25
        ]
    ...
    def _is_valid_all_idle_workers_gather_minerals(self, dc):
        if (len(dc.idle_units_of_type(UNIT_TYPE.ZERG_DRONE.value)) > 0 and len(dc.minerals) > 0):
            return True
        else:
            return False
    ...
    def _assign_workers_gather_gas(self, dc):
        ...
        workers = [
            u for u in dc.units_of_type(UNIT_TYPE.ZERG_DRONE.value) if (
                len(u.orders) == 0 or (
        ...
        action = sc_pb.Action()
        action.action_raw.unit_command.unit_tags.extend([u.tag for u in assigned_workers])
        action.action_raw.unit_command.ability_id = \
            ABILITY.HARVEST_GATHER_DRONE.value
    ...
    def _is_valid_assign_workers_gather_gas(self, dc):
        ...
        if len(idle_extractors) > 0 and len(workers) > 0:
            return True
        else:
            return False
