...
class UpgradeActions(object):
    ...
    def action(self, func_name, upgrade_id):
        return Function(
            name=func_name, function=self._upgrade_unit(upgrade_id), is_valid=self._is_valid_upgrade_unit(upgrade_id)
        )
    ...
    def _is_valid_upgrade_unit(self, upgrade_id):
        def is_valid(dc):
            ...
            if (has_required_units and has_required_upgrades and upgrade_id not in dc.upgraded_techs
                    and len(dc.units_with_task(tech.buildAbility)) == 0 and dc.mineral_count >= tech.mineralCost
                    and dc.gas_count >= tech.gasCost and dc.supply_count >= tech.supplyCost
                    and len(dc.idle_units_of_types(tech.whatBuilds)) > 0):
                return True
            else:
                return False
