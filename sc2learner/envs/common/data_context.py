...
class DataContext(object):
    ...
    def reset(self, observation):
        ...
        self._init_base_pos = (init_base.float_attr.pos_x, init_base.float_attr.pos_y)
    ...
    def mature_units_of_type(self, type_id, ally=ALLY_TYPE.SELF.value):
        return [u for u in self.units_of_type(type_id, ally) if u.float_attr.build_progress >= 1.0]

    def idle_units_of_type(self, type_id, ally=ALLY_TYPE.SELF.value):
        return [u for u in self.mature_units_of_type(type_id, ally) if len(u.orders) == 0]
    ...
    def mature_units_of_types(self, type_list, ally=ALLY_TYPE.SELF.value):
        return [u for u in self.units_of_types(type_list, ally) if u.float_attr.build_progress >= 1.0]

    def idle_units_of_types(self, type_list, ally=ALLY_TYPE.SELF.value):
        return [u for u in self.mature_units_of_types(type_list, ally) if len(u.orders) == 0]

    def units_with_task(self, ability_id, ally=ALLY_TYPE.SELF.value):
        return [u for u in self.units_of_alliance(ally) if ability_id in set([order.ability_id for order in u.orders])]
    ...
    def minerals(self):
        return [
            u for u in self._units if (
                u.unit_type == UNIT_TYPE.NEUTRAL_MINERALFIELD.value
                or u.unit_type == UNIT_TYPE.NEUTRAL_MINERALFIELD750.value
            )
        ]

    @property
    def unexploited_minerals(self):
        self_bases = self.units_of_types(
            [UNIT_TYPE.ZERG_HATCHERY.value, UNIT_TYPE.ZERG_LAIR.value, UNIT_TYPE.ZERG_HIVE.value]
        )
        enemy_bases = self.units_of_types(
            [UNIT_TYPE.ZERG_HATCHERY.value, UNIT_TYPE.ZERG_LAIR.value, UNIT_TYPE.ZERG_HIVE.value], ALLY_TYPE.ENEMY.value
        )
        return [u for u in self.minerals if utils.closest_distance(u, self_bases + enemy_bases) > 15]

    @property
    def gas(self):
        return [u for u in self._units if u.unit_type == UNIT_TYPE.NEUTRAL_VESPENEGEYSER.value]
    ...
    def exploitable_gas(self):
        ...
        bases = self.mature_units_of_types(
            [UNIT_TYPE.ZERG_HATCHERY.value, UNIT_TYPE.ZERG_LAIR.value, UNIT_TYPE.ZERG_HIVE.value]
        )
        return [
            u for u in self.gas
            if (utils.closest_distance(u, bases) < 10 and utils.closest_distance(u, extractors) > 3)
        ]
