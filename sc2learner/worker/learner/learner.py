...
class HistoryActorInfo(object):
    def __init__(self, actor_monitor_arg):
        ...
        self.game_results = {
            k: deque(maxlen=actor_monitor_arg.difficulty_queue_len)
    ...
    def state_dict(self):
        return {'actor_data': self._data, 'game_results': self.game_results, 'game_lengths': self.game_lengths}
...
class BaseLearner(object):
    def __init__(self, env, model, cfg=None):
        ...
        self.dataset = OnlineDataset(
            data_maxlen=cfg.train.learner_data_queue_size,
        ...
        if 'IN_K8S' in os.environ:
            pull_port = os.getenv('SENSESTAR_LEARNER_SERVICE_SERVICE_PORT_LEARNER', port['learner'])
            rep_port = os.getenv('SENSESTAR_LEARNER_SERVICE_SERVICE_PORT_LEARNER_MODEL', port['learner_manager_model'])
        else:
            pull_port = port['learner']
            rep_port = port['learner_manager_model']
        self.HWM = cfg.communication.HWM['learner']
        self.pull_thread = Thread(target=self._pull_data, args=(self.zmq_context, pull_port))
        self.reply_model_thread = Thread(target=self._reply_model, args=(self.zmq_context, rep_port))
    ...
    def _record_info(self, iterations):
        if iterations % self.cfg.logger.print_freq == 0:
            ...
            self.tb_logger.add_histogram(
                'data_rollout_time', self.history_actor_info.get_distribution('data_rollout_time'), iterations
            )
            self.tb_logger.add_histogram(
                'update_model_time', self.history_actor_info.get_distribution('update_model_time'), iterations
            )
            self.tb_logger.add_histogram('game_step', self.history_actor_info.get_distribution('step'), iterations)
            self.tb_logger.add_histogram('game_length', self.history_actor_info.get_length_distribution(), iterations)
            self.tb_logger.add_histogram(
                'dataset_staleness', np.array([iterations - d['model_index'] for d in self.dataset.data_queue]),
                iterations
            )
        ...
        if iterations % self.cfg.logger.save_freq == 0:
            self.checkpoint_helper.save_iterations(
                iterations,
                self.model,
                optimizer=self.optimizer,
                dataset=self.dataset,
