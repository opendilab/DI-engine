...
class AlphaStarActorWorker(AlphaStarActor):
    ...
    def _module_init(self):
        self.context = ActorContext(self.cfg, self.logger, self.actor_id)
...
class ActorContext:
    def __init__(self, cfg, logger, actor_id):
        self.cfg = cfg
...
class JobGetter:
    ...
    def get_job(self, actor_id):
        ...
        try:
            while True:
                d = {'request_id': self.job_request_id, 'actor_id': actor_id}
...
class DataPusher:
    ...
    def push(self, metadata, data_buffer):
        ...
        d = {'actor_id': self.context.actor_id, 'job_id': job_id, 'metadata': metadata}
...
class ModelLoader:
    ...
    def load_model(self, job, agent_no, model):
        ...
        self.context.logger.info('Model {} loaded for agent {}, time:{}'
                                 .format(path, agent_no, time.time() - t))
...
class StatRequester:
    ...
    def request_stat(self, job, agent_no):
        stat = torch.load(self._get_stat_ceph_path(job['stat_id'][agent_no]), map_location='cpu')
        return stat
