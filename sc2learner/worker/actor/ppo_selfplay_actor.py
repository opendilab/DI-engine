...
class PpoSelfplayActor(BaseActor):
    def __init__(self, *args, **kwargs):
        super(PpoSelfplayActor, self).__init__(*args, **kwargs)
    ...
    def _nstep_rollout(self):
        ...
        for _ in range(self.unroll_length):
            ...
            with torch.no_grad():
                ...
                oppo_model_output = self.oppo_model(inputs, mode='step')
            action = self._process_model_output(model_output, outputs)
            oppo_action = self._process_model_output(oppo_model_output, outputs)
            self.obs, reward, self.done, info = self.env.step([action, oppo_action])
    ...
    # overwrite
    def _init(self):
        self.obs, self.oppo_obs = self.env.reset()
        self.done = False
