...
class Model(object):
    def __init__(
        ...
        *,
        ...
        act_model = policy(sess, scope_name, ob_space, ac_space, nbatch_act, 1, reuse=False)
        train_model = policy(sess, scope_name, ob_space, ac_space, nbatch_train, unroll_length, reuse=True)
        ...
        def train(lr, cliprange, obs, returns, dones, actions, values, neglogpacs, states=None):
            advs = returns - values
    ...
    def __init__(
        ...
        if enable_push:
            ...
            self._push_thread = Thread(
                target=self._push_data, args=(self._zmq_context, learner_ip, port_B, self._data_queue)
            )
    ...
    def _nstep_rollout(self):
        ...
        for _ in range(self._unroll_length):
            action, value, self._state, neglogpac = self._model.step(
                transform_tuple(self._obs, lambda x: np.expand_dims(x, 0)), self._state, np.expand_dims(self._done, 0)
            )
        ...
        if isinstance(self._obs, tuple):
            mb_obs = tuple(np.asarray(obs, dtype=self._obs[0].dtype) for obs in zip(*mb_obs))
        else:
        ...
        return (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states, episode_infos)
...
class PPOLearner(object):
    def __init__(
        ...
        self._pull_data_thread = Thread(
            ...
            args=(self._zmq_context, self._data_queue, self._episode_infos, self._unroll_split, port_B)
        )
        self._pull_data_thread.start()
        self._reply_model_thread = Thread(target=self._reply_model, args=(self._zmq_context, port_A))
    ...
    def run(self):
        ...
        batch_threads = [
            Thread(
                target=self._prepare_batch, args=(self._data_queue, batch_queue, self._batch_size * self._unroll_split)
            ) for _ in range(8)
        ]
        ...
        while True:
            while (self._learn_act_speed_ratio > 0
                   and updates * self._batch_size >= self._num_unrolls * self._learn_act_speed_ratio):
                time.sleep(0.001)
            updates += 1
            ...
            loss.append(
                self._model.train(lr_now, clip_range_now, obs, returns, dones, actions, values, neglogpacs, states)
            )
            self._model_params = self._model.read_params()
            ...
            if updates % self._print_interval == 0:
                ...
                tprint(
                    ...
                    (updates, train_fps, rollout_fps, var, avg_reward, *loss_mean[:5], time_elapsed)
                )
            ...
            if unroll_split > 1:
                data_queue.extend(
                    list(
                        zip(
                            *(
                                [list(zip(*transform_tuple(data[0], lambda x: np.split(x, unroll_split))))] +
                                [np.split(arr, unroll_split)
                                 for arr in data[1:-2]] + [[data[-2] for _ in range(unroll_split)]]
                            )
                        )
                    )
                )
...
class PPOAgent(object):
    ...
    def act(self, observation):
        action, value, self._state, _ = self._model.step(
            transform_tuple(observation, lambda x: np.expand_dims(x, 0)), self._state, np.expand_dims(self._done, 0)
        )
        return action[0]
...
class PPOSelfplayActor(object):
    ...
    def _nstep_rollout(self):
        ...
        for _ in range(self._unroll_length):
            ...
            oppo_action, _, self._oppo_state, _ = self._oppo_model.step(
                transform_tuple(self._oppo_obs, lambda x: np.expand_dims(x, 0)), self._oppo_state,
            ...
            mb_neglogpacs.append(neglogpac[0])
            mb_dones.append(self._done)
            (self._obs, self._oppo_obs), reward, self._done, info = self._env.step([action[0], oppo_action[0]])
            self._cum_reward += reward
            if self._done:
    ...
    def _update_model(self):
        ...
        if (not self._freeze_opponent_pool and random.uniform(0, 1.0) < self._model_cache_prob):
            self._model_cache.append(model_params)
        self._latest_model = model_params

    def _update_opponent(self):
        if (random.uniform(0, 1.0) < self._prob_latest_opponent or len(self._model_cache) == 0):
            ...
            tprint("Opponent updated with the previous model. %d models cached." % len(self._model_cache))
