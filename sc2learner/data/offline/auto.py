...
def check_if_exist(output_dir, replay_name_prefix, count=2):
    if finish_list == {}:
        for f in os.listdir(output_dir):
            ff = f.strip().split('.')[0].split('_')[-1]
            if ff not in finish_list:
                finish_list[ff] = 0
            finish_list[ff] += 1
    try:
        c = finish_list[replay_name_prefix]
        if c == count:
            return True
        else:
            return False
...
def main_4_9():
        ...
        if k in partitions:
            ...
            for delete_workstation in delete_workstations:
                if delete_workstation in v:
                    total_list_num -= 1
    todo_list_path = '/mnt/lustre/zhangming/data/list/decode_replay/decoded.4.9.3.list.0508.todo'
    # todo_list_path = '/mnt/lustre/zhangming/data/list/zmy/zerg_7500.list.0508.todo'
    replays = open(todo_list_path, 'r').readlines()
    # replays = open('/mnt/lustre/zhangming/data/listtemp410/replays.raw.list.5200', 'r').readlines()
    ...
    for partition in partitions:
        ...
        if partition != 'VI_SP_Y_V100_A':
            ...
            workstations = info[partition]
        for workstation in workstations:
            if replays_index < total_list_num and workstation not in delete_workstations:
