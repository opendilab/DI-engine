...
from dizoo.procgen.coinrun.entry.coinrun_ppg_config import coinrun_ppg_default_config


def main(cfg, seed=0, max_iterations=int(1e10)):
    cfg.exp_name = 'coinrun_ppg'
    cfg = compile_config(
        cfg,
    ...
    collector_env = SyncSubprocessEnvManager(
        env_fn=[lambda: CoinRunEnv(cfg.env) for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    evaluator_env = SyncSubprocessEnvManager(
        env_fn=[lambda: CoinRunEnv(cfg.env) for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
...
if __name__ == "__main__":
    main(EasyDict(coinrun_ppg_default_config))
