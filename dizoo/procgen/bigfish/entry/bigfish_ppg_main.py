...
from dizoo.procgen.bigfish.envs import BigfishEnv
from dizoo.procgen.bigfish.entry.bigfish_ppg_config import bigfish_ppg_default_config


def main(cfg, seed=0, max_iterations=int(1e10)):
    cfg.exp_name = 'bigfish_ppg'
    cfg = compile_config(
        cfg,
    ...
    collector_env = SyncSubprocessEnvManager(
        env_fn=[lambda: BigfishEnv(cfg.env) for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    evaluator_env = SyncSubprocessEnvManager(
        env_fn=[lambda: BigfishEnv(cfg.env) for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
...
if __name__ == "__main__":
    main(EasyDict(bigfish_ppg_default_config))
