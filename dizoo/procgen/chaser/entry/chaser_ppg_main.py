...
from dizoo.procgen.chaser.envs import ChaserEnv
from dizoo.procgen.chaser.entry.chaser_ppg_config import chaser_ppg_default_config


def main(cfg, seed=0, max_iterations=int(1e10)):
    cfg.exp_name = 'chaser_ppg'
    cfg = compile_config(
        cfg,
    ...
    collector_env = SyncSubprocessEnvManager(
        env_fn=[lambda: ChaserEnv(cfg.env) for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    evaluator_env = SyncSubprocessEnvManager(
        env_fn=[lambda: ChaserEnv(cfg.env) for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
...
if __name__ == "__main__":
    main(EasyDict(chaser_ppg_default_config))
