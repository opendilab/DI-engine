...
#OLD Exp
P1 = [1e-3, 1.5e-3, 2e-3, 5e-2, 0.5, 1]
P2 = [5e-3, 5.5e-3, 9e-3, 0.2, 0.8, 1]
P3 = [3e-3, 3.5e-3, 6e-3, 0.1, 0.6, 1]
packet_arrive_lambda_all = [50, 100]
    ...
    def detect_receive_results(self, action, agent_id):
        if self.send[agent_id]:
            if self.codebooks[action[agent_id]]<len(P1):
                if agent_id < int(IoT_users/2) and action[agent_id] < int(Codebooks_Num/2):
                    P = P1
                elif agent_id >= int(IoT_users/2) and action[agent_id] < int(Codebooks_Num/2):
                    P = P2
                else:
                    P = P3
