    ...
    def _make_env(self, seed=None, save_video=False, save_path=None, video_callable=None, uid=None, test=False,
        ...
        if test:
            if final_test:
                ...
                max_moves = self.cfg.test_max_episode_steps
            env = make_atari(self.env_name, skip=self.frame_skip, max_episode_steps=max_moves)
        else:
            env = make_atari(self.cfg.env_name, skip=self.cfg.frame_skip, max_episode_steps=self.cfg.max_episode_steps)
        ...
        return AtariWrapper(env, cvt_string=self.cfg.cvt_string)
