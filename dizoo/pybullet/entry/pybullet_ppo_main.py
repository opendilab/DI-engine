...
from dizoo.pybullet.envs.pybullet_env import PybulletEnv
from dizoo.classic_control.pendulum.config.pendulum_ppo_config import pendulum_ppo_config
from dizoo.pybullet.config.hopper_ppo_default_config import hopper_ppo_default_config
...
def main(cfg, seed=0, max_iterations=int(1e10)):
    ...
    collector_env = BaseEnvManager(
        env_fn=[lambda: PybulletEnv(cfg.env) for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    evaluator_env = BaseEnvManager(
        env_fn=[lambda: PybulletEnv(cfg.env) for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
