...
from ding.model import DiscreteHead, ConvEncoder
from ding.utils import list_split


class GoBiggerHybridNetwork(nn.Module):
    ...
    def __init__(
            ...
            encoder_output_size_list=[256, 128, 128],
            rnn: bool = False,
            activation=nn.ReLU(),
    ) -> None:
        super(GoBiggerHybridNetwork, self).__init__()
        self.activation = activation
        self.action_type_shape = action_type_shape

        spatial_output_size, scalar_output_size, unit_output_size = encoder_output_size_list
        assert unit_output_size == 128, "Not Implemented"
        # encoder
        self.spatial_encoder = ConvEncoder(spatial_shape, [64, 128, spatial_output_size], activation=self.activation)
    ...
    def forward(self, obs: list) -> dict:
        ...
        """
        B = obs[0]['spatial_obs'].shape[0]
        A = len(obs)
        # aggreate agent dim
        spatial_obs = torch.stack([o['spatial_obs'] for o in obs], dim=1)
        spatial_obs = spatial_obs.view(B * A, *spatial_obs.shape[2:])
        ...
        spatial_feature = self.spatial_encoder(spatial_obs)  # B, N3

        fused_embedding_total = torch.cat([spatial_feature, scalar_feature, unit_feature.mean(dim=1)], dim=1)
        if self.rnn:
            prev_state = []
            for b in range(B):
                for a in range(A):
