...
from ding.envs.env_manager.async_vector_env_manager import AsyncVectorEnvManager
...
def main(cfg, seed=0):
    ...
    collector_env = AsyncVectorEnvManager(
        env_fn=[wrapped_cartpole_env for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    evaluator_env = AsyncVectorEnvManager(
        env_fn=[wrapped_cartpole_env for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
