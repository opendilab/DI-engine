...
def main(cfg, seed=0):
    ...
    collector_env = BaseEnvManager(
        env_fn=[lambda: DingEnvWrapper(env=gym.make('Moving-v0'), cfg=cfg.env) for _ in range(collector_env_num)],
        cfg=cfg.env.manager
    )
    evaluator_env = BaseEnvManager(
        env_fn=[lambda: DingEnvWrapper(env=gym.make('Moving-v0'), cfg=cfg.env) for _ in range(evaluator_env_num)],
        cfg=cfg.env.manager
    )
    ...
    # evaluate
    evaluator_env = BaseEnvManager(env_fn=[lambda: DingEnvWrapper(env=gym.make('Moving-v0'), cfg=cfg.env) for _ in range(evaluator_env_num)],
        cfg=cfg.env.manager)
    evaluator_env.enable_save_replay(cfg.env.replay_path)  # switch save replay interface
    evaluator = InteractionSerialEvaluator(
        cfg.policy.eval.evaluator, evaluator_env, policy.eval_mode, tb_logger, exp_name=cfg.exp_name
