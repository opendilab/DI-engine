...
from dizoo.d4rl.config.hopper_medium_edt_config import main_config, create_config
...
def main():
    ...
    with task.start(async_mode=False, ctx=OfflineRLContext()):
        ...
        model = ElasticDecisionTransformer(**cfg.policy.model)
        policy = EDTPolicy(cfg.policy, model=model)
